...
<div className="mb-3 ml-32 mt-6">
  ...
  <motion.select whileHover={{ scale: 1.3}}
    ...
    <motion.div whileHover={{scale:1.2, boxShadow: "8px 8px 8px rgb(100,100,100)",}} key={getAllItems._id} class='bg-gray-300   border-4 border-blue-900 rounded-xl p-3 mr-28 mt-20 text-center text-2xl font-bold'>
       
        
        <label class='block mb-4 text-[15px]'> {item.Inventry_Item_DisplayName}</label>
       <Link to={"/ItemView/"+item._id}><label><motion.img whileHover={{scale:1.1, textShadow: ""}} src={`http://localhost:8080${item.Image}`} class="w-28 h-30 " /></label><br/></Link> 
        <label class='block mb-6 text-[20px]'> Rs. {item.Inventry_Item_Price*(100-item.Inventry_Item_Discount)/100}</label>
